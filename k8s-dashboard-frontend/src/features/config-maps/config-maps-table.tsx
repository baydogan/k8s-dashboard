...
export function ConfigMapsTable({ items }: { items: ConfigMap[] }) {
  ...
  const filtered = useMemo(() =>
    items.filter((item) => {
      ...
      const match = !q || item.name.toLowerCase().includes(q) || item.namespace.toLowerCase().includes(q) || item.dataKeys.some((k) => k.toLowerCase().includes(q));
      return match && (!ns || item.namespace === ns);
    }),
  [items, search, ns]);

  const headers = [t("columns.name"), t("columns.namespace"), t("columns.keys"), t("columns.age"), ""];
                ...
                <tr key={`${item.namespace}/${item.name}`} className="hover:bg-bg-sunken/50 transition-colors group cursor-pointer animate-slide-up" style={{ animationDelay: `${i * 30}ms` }}>
                  ...
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-1.5 flex-wrap">
                      {item.dataKeys.slice(0, 4).map((k) => (
                        <span key={k} className="font-mono text-[10px] bg-bg-sunken border border-border text-text-muted rounded px-1.5 py-0.5">{k}</span>
                      ))}
                      {item.dataKeys.length > 4 && <span className="font-mono text-[10px] text-text-dim">+{item.dataKeys.length - 4}</span>}
                    </div>
                  </td>
