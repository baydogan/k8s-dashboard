...
import type { StatefulSet, StatefulSetStatus } from "@/entities/statefulset";

function statusToVariant(s: StatefulSetStatus) {
  switch (s) {
    case "Running":  return "success"  as const;
    case "Updating": return "info"     as const;
    case "Degraded": return "warning"  as const;
    case "Failed":   return "critical" as const;
  }
}

function statusToDot(s: StatefulSetStatus): "healthy" | "warning" | "critical" | "pending" {
  switch (s) {
...
export function StatefulSetsTable({ items }: { items: StatefulSet[] }) {
  ...
  const headers = [t("columns.name"), t("columns.namespace"), t("columns.status"), t("columns.ready"), t("columns.service"), t("columns.storage"), t("columns.age"), ""];
                ...
                <tr key={`${item.namespace}/${item.name}`} className="hover:bg-bg-sunken/50 transition-colors group cursor-pointer animate-slide-up" style={{ animationDelay: `${i * 30}ms` }}>
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-2.5">
                      <StatusDot status={statusToDot(item.status)} pulse={item.status === "Updating"} />
                  ...
                  <td className="px-4 py-3"><Badge variant={statusToVariant(item.status)}>{t(`status.${item.status.toLowerCase() as "running" | "degraded" | "failed" | "updating"}`)}</Badge></td>
                  <td className="px-4 py-3">
                  ...
                  <td className="px-4 py-3 font-mono text-xs text-text-muted">{item.serviceName}</td>
                  <td className="px-4 py-3 font-mono text-xs text-text-dim">{item.storageClass ?? "—"}</td>
