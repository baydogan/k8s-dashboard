...
export function NetworkPoliciesTable({ items }: { items: NetworkPolicy[] }) {
  const [search, setSearch] = useState("");
  const [ns, setNs] = useState("");
  const t = useTranslations("networkPolicies");
  ...
  const filtered = useMemo(() =>
    items.filter((item) => {
      ...
      const match = !q || item.name.toLowerCase().includes(q) || item.namespace.toLowerCase().includes(q) || item.podSelector.toLowerCase().includes(q);
      return match && (!ns || item.namespace === ns);
    }),
  [items, search, ns]);

  const headers = [t("columns.name"), t("columns.namespace"), t("columns.podSelector"), t("columns.policyTypes"), t("columns.ingress"), t("columns.egress"), t("columns.age"), ""];
            ...
            <tbody className="divide-y divide-border">
              {filtered.length === 0 ? (
                ...
                <tr key={`${item.namespace}/${item.name}`} className="hover:bg-bg-sunken/50 transition-colors group cursor-pointer animate-slide-up" style={{ animationDelay: `${i * 30}ms` }}>
                  ...
                  <td className="px-4 py-3 font-mono text-xs text-text-dim">{item.podSelector}</td>
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-1">
                      {item.policyTypes.map((pt) => <Badge key={pt} variant={pt === "Ingress" ? "info" : "warning"}>{pt}</Badge>)}
                    </div>
                  </td>
                  <td className="px-4 py-3 font-mono text-xs text-text-muted">{item.ingressRules}</td>
                  <td className="px-4 py-3 font-mono text-xs text-text-muted">{item.egressRules}</td>
