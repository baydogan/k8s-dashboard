...
export function RoleBindingsTable({ items }: { items: RoleBinding[] }) {
  const [search, setSearch] = useState("");
  const [ns, setNs] = useState("");
  const t = useTranslations("roleBindings");
  ...
  const headers = [t("columns.name"), t("columns.namespace"), t("columns.roleRef"), t("columns.roleKind"), t("columns.subjects"), t("columns.age"), ""];
                ...
                <tr key={`${item.namespace}/${item.name}`} className="hover:bg-bg-sunken/50 transition-colors group cursor-pointer animate-slide-up" style={{ animationDelay: `${i * 30}ms` }}>
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-2">
                      <Link className="h-3.5 w-3.5 text-text-dim shrink-0" />
                  ...
                  <td className="px-4 py-3">
                    <Badge variant={item.roleKind === "ClusterRole" ? "info" : "muted"}>{item.roleKind}</Badge>
                  </td>
