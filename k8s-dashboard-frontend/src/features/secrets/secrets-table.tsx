...
import type { Secret, SecretType } from "@/entities/secret";

function typeVariant(t: SecretType) {
  switch (t) {
    case "kubernetes.io/tls":                   return "info"    as const;
    case "kubernetes.io/dockerconfigjson":      return "warning" as const;
    case "bootstrap.kubernetes.io/token":       return "warning" as const;
    default:                                    return "muted"   as const;
  }
}

function typeLabel(t: SecretType) {
  switch (t) {
    ...
    case "bootstrap.kubernetes.io/token":       return "Bootstrap";
    default:                                    return "Opaque";
  }
}

export function SecretsTable({ items }: { items: Secret[] }) {
  ...
  const filtered = useMemo(() =>
    items.filter((item) => {
      ...
      const match = !q || item.name.toLowerCase().includes(q) || item.namespace.toLowerCase().includes(q) || typeLabel(item.type).toLowerCase().includes(q);
      return match && (!ns || item.namespace === ns);
    }),
  [items, search, ns]);

  const headers = [t("columns.name"), t("columns.namespace"), t("columns.type"), t("columns.keys"), t("columns.age"), ""];
                ...
                <tr key={`${item.namespace}/${item.name}`} className="hover:bg-bg-sunken/50 transition-colors group cursor-pointer animate-slide-up" style={{ animationDelay: `${i * 30}ms` }}>
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-2">
                      <Lock className="h-3 w-3 text-text-dim shrink-0" />
                  ...
                  <td className="px-4 py-3"><Badge variant={typeVariant(item.type)}>{typeLabel(item.type)}</Badge></td>
                  <td className="px-4 py-3 font-mono text-xs text-text-muted">{item.dataCount}</td>
