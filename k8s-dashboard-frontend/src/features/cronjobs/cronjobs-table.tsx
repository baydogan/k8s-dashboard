...
import type { CronJob, CronJobStatus } from "@/entities/cronjob";

function statusToVariant(s: CronJobStatus) {
  return s === "Active" ? "success" as const : "muted" as const;
}

export function CronJobsTable({ items }: { items: CronJob[] }) {
  ...
  const filtered = useMemo(() =>
    items.filter((item) => {
      ...
      const match = !q || item.name.toLowerCase().includes(q) || item.namespace.toLowerCase().includes(q) || item.schedule.toLowerCase().includes(q);
      return match && (!ns || item.namespace === ns);
    }),
  [items, search, ns]);

  const headers = [t("columns.name"), t("columns.namespace"), t("columns.schedule"), t("columns.lastSchedule"), t("columns.lastSuccessful"), t("columns.active"), t("columns.age"), ""];
                ...
                <tr key={`${item.namespace}/${item.name}`} className="hover:bg-bg-sunken/50 transition-colors group cursor-pointer animate-slide-up" style={{ animationDelay: `${i * 30}ms` }}>
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-2.5">
                      <StatusDot status={item.status === "Active" ? "healthy" : "unknown"} />
                  ...
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-2">
                      <Badge variant={statusToVariant(item.status)}>{t(`status.${item.status === "Active" ? "active" : "suspended"}`)}</Badge>
                      <span className="font-mono text-xs text-text-dim">{item.schedule}</span>
                    </div>
                  </td>
                  <td className="px-4 py-3 font-mono text-xs text-text-muted">{item.lastSchedule ?? t("noData")}</td>
                  <td className="px-4 py-3 font-mono text-xs text-text-muted">{item.lastSuccessful ?? t("noData")}</td>
                  <td className="px-4 py-3">
                    <span className={`font-mono text-xs ${item.activeJobs > 0 ? "text-accent-info" : "text-text-dim"}`}>{item.activeJobs}</span>
                  </td>
