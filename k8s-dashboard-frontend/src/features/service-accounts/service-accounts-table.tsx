...
export function ServiceAccountsTable({ items }: { items: ServiceAccount[] }) {
  ...
  const headers = [t("columns.name"), t("columns.namespace"), t("columns.secrets"), t("columns.age"), ""];
                ...
                <tr key={`${item.namespace}/${item.name}`} className="hover:bg-bg-sunken/50 transition-colors group cursor-pointer animate-slide-up" style={{ animationDelay: `${i * 30}ms` }}>
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-2">
                      <UserCircle className="h-3.5 w-3.5 text-text-dim shrink-0" />
                  ...
                  <td className="px-4 py-3 font-mono text-xs text-text-muted">{item.secrets}</td>
