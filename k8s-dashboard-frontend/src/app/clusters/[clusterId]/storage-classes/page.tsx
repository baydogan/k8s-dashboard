...
import { StorageClassesTable } from "@/features/storage-classes/storage-classes-table";
import { getStorageClasses } from "@/services/storage-class-service";
import { getTranslations } from "next-intl/server";

interface Props { params: Promise<{ clusterId: string }> }

export default async function StorageClassesPage({ params }: Props) {
  const { clusterId } = await params;
  const items = await getStorageClasses(clusterId);
  const t = await getTranslations("storageClasses");
  ...
  return (
    <>
      ...
      <PageContainer>
        ...
        <StorageClassesTable items={items} />
      </PageContainer>
    </>
  );
}
